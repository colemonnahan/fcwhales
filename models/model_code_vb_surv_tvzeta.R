...
## this version also has a random effect on annual birth prob, which is
## setup to be fairly uninformative on the ilogit scale. Per request of a
## reviewer (Cole added 8/2018).
...
model{
    ...
    for(yr in 2:nyrs){ # loop over year
      ...
      piab[ind,yr] <-
        ...
        ifelse(yr==nyrs,
               ...
               sazero[ind,yr]*zeta[yr] + (1-sazero[ind,yr]))
      piad[ind,yr] <-
  ...
  ## This model differs from the other versions b/c zeta is a random effect
  ## in ilogit space instead of constant across years. These hyperpriors
  ## which give about a uniform on the inverse logit (probability)
  ## scale. I.e., resemble U(0,1).
  mu_zeta ~ dnorm(0,1)
  sd_zeta ~ dnorm(0,2)T(0,)
  prec_zeta <- 1/(sd_zeta*sd_zeta)
  ## Likelihood of annual zeta. Year 1 is zeta0 above and last
  ## year is exactly 1 so those will not affect the likelihood and thus
  ## take the hyperdistribution exactly.
  for(yr in 1:nyrs){
    tau_zeta[yr] ~ dnorm(mu_zeta, prec_zeta)
    logit(zeta[yr]) <- tau_zeta[yr]
  }
